...
def detect_circle_new(image_pil: Image, name: str = '', path_num: str = '', path_char: str = '', mapping_azure: bool = False):
    # Find circles
    rmin = 15
    rmax = 40
    steps = 100
...
def mapping_code(raw_image_pil: Image, words: List, boxes: List, det_df: pd.DataFrame, 
    ...
    for row in range(num_cell):
        for column in range(num_cell):
            ...
            circle_box = detect_circle(crop_image, mapping_azure= True)
            code_str = ''
            note_str = ''
            if circle_box:
...
def refine_format(data_dict: Dict, floor_str: str) -> Dict:
    ...
    Args:
        data_dict (Dict): A dictionary containing keys 'code' and 'digit_code'. The 'code' key holds a list
        of strings, and the 'digit_code' key holds a list of integers

        floor_str (str): Floor_str is a string that represents the floor number in a building. It is used
        as part of the key in the output dictionary to indicate the floor number along with the code
        associated with that floor

    Returns:
        Dict: The function `refine_format` returns a dictionary where the keys are formatted as
...
def crop_image_and_save(coordinate, image):
    ...
    Args:
        coordinate (tuple): The `coordinate` parameter in the `crop_image_by_list_and_save` function is a
        ...
        image (Image): The `image` parameter in the `crop_image_by_list_and_save` function is expected to be
...
def crop_image_by_coordinate(coordinate_point, image, is_save= False, path = ""):
    ...
    Args:
        ...
        coordinate_point (tuple) : The `coordinate_point` parameter is a tuple that represents the coordinates
        of the bounding box to crop the image. It should have 4 values: (x1, y1, x2, y2), where (x1, y1) are
        the coordinates of the top-left corner
        
        image (Image) : The `image` parameter in the `crop_image_by_coordinate` function is expected to be an
        ...
        is_save (bool) : The `is_save` parameter in the `crop_image_by_coordinate` function is a boolean flag
        ...
        path (str) : The `path` parameter in the `crop_image_by_coordinate` function is a string that
    ...
    Return:
        If the `is_save` parameter is `False`, the function will return the cropped image.
    """
...
def convert_from_image_to_cv2(img: Image) -> np.ndarray:
    """
    The function `convert_from_image_to_cv2` converts an image from the `PIL` format to a `numpy` array
    using OpenCV.
    
    Args:
        img (Image) : The `img` parameter is an image object that you want to convert to a NumPy array using
        OpenCV (cv2)

    Return: 
        a NumPy array representing the image converted from the RGB color space to the BGR color
        space using OpenCV.
    """
...
def reformat_code(result: str = ''):
    """
    The function reformat_code adds specific formatting to the input string if it starts with 'K', 'M',
    or 'SH'.
    
    Args:
        Result (str) : The `reformat_code` function takes a string `result` as input and checks if it starts
        with 'K', 'M', or 'SH'. If it does, it adds some formatting to the string by adding '\t\t\t\t\t上' at
        the beginning and '

    Return: 
        The function `reformat_code` returns the modified `result` string with additional
        formatting if it starts with 'K', 'M', or 'SH', otherwise it returns the original `result` string.
    """
...
def upscale_image(image_path: str, restorer,is_saved: bool = True, des_path: str = ''):
    """
    This function takes an image file path, a restorer object, and optional parameters to enhance the
    image and save the restored image to a specified destination path.
    
    Args:
        image_path (str): The `image_path` parameter is a string that represents the file path of the image
        that you want to upscale

        restorer (GFP GAN model): The `restorer` parameter in the `upscale_image` function is likely an object or
        instance of a class that has a method named `enhance`. This method seems to take several arguments
        such as `input_img`, `has_aligned`, `only_center_face`, `paste_back`, and `
    
        is_saved (Bool) : The `is_saved` parameter in the `upscale_image` function is a boolean parameter
        that determines whether the restored image should be saved or not. If `is_saved` is set to `True`,
        the restored image will be saved to the specified destination path. If set to `False`, the, defaults
        to True
    
        des_path (str): The `des_path` parameter in the `upscale_image` function is used to specify the
        destination path where the upscaled/restored image will be saved. This parameter should be a string
        representing the directory path where you want to save the image

    Return: 
        The function `upscale_image` is returning the path where the restored image is saved.
    """
...
def extract_output(model, image: Image):
    """
    This function takes an image as input, processes it through a model, and returns the predicted label
    for the text in the image.
    
    Args:
        model (ParSeq model): The `model` parameter is likely an object representing a machine learning model,
        possibly a text recognition model for scene text. It is used to make predictions on the input image
    
        image (Image) : The `image` parameter is expected to be an image input that you want to process using
        the `extract_output` function. This image will be transformed using the `img_transform` method,
        passed through the `model`, and the output will be decoded to obtain the label and confidence of the
        prediction
    
    Return: 
        the predicted label for the input image.
    """
...
def preprocessing_str(basic_info_dict: Dict = {}) -> Dict:
    ...
    Args:
        basic_info_dict (Dict) : The function `preprocessing_str` takes a dictionary `basic_info_dict` as
    ...
    Return: 
        The function `preprocessing_str` is returning the `basic_info_dict` after performing some
...
def text_recognition(img: Image, model, get_num_only = True):
    """
    This Python function takes an image, a model for text recognition, and an option to extract only
    numbers, and returns the recognized text or numbers from the image.
    
    Args:
        img (PIL) : The `img` parameter is expected to be an image object that you want to perform text
        recognition on

        model (Paddle model) : The `model` parameter in the `text_recognition` function is used for performing
        optical character recognition (OCR) on the input image. It is expected to be an OCR model that can
        process images and extract text information from them. The function uses this model to recognize
        text in the image provided as
    
        get_num_only (Bool) : The `get_num_only` parameter in the `text_recognition` function is a boolean
        flag that determines whether the function should extract and return only numbers from the recognized
        text. If `get_num_only` is set to `True`, the function will extract numbers from the text and return
        them. If, defaults to True (optional)

    Return: 
        The function `text_recognition` returns a string that represents the recognized text from
        the input image. If `get_num_only` is set to True, it returns only the extracted numbers from the
        text. If `get_num_only` is set to False, it returns the first non-numeric word recognized from the
        text. If no text is recognized or no non-numeric word is found, an
    """
...
def text_detect(image_path: str, path_temp: str, model, parseq_model) -> None:
    ...
    Args:
        image_path (str) : The `image_path` parameter is a string that represents the file path to the image
        ...
        path_temp (str) : `path_temp` is a string parameter that represents the path where temporary files
        will be saved during the execution of the `text_detect` function
    
        model (Paddle model): The `model` parameter in the `text_detect` function seems to be used for optical
        ...
        parseq_model (ParSeq model): The `parseq_model` parameter in the `text_detect` function seems to be a model
    ...
    Return: 
        The function `text_detect` returns a tuple containing two values. The first value is the
    ...
    if result[0] is None:
        ...
        for idx in range(len(result[0])):
            ...
            try:
                ...
                word_recognition_1st = text_recognition(img= word,
                                                        model= model,
                                                        get_num_only= False)
                if word_recognition_1st == '':
                ...
                elif word_recognition_1st == 'float':
                    continue
                else:
                    word.save(f"{path_temp}/{name_file}")
                    return word_recognition_1st, f"{path_temp}/{name_file}"
            except:
                continue
        return '', ''
